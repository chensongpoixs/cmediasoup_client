...
static bool nvenc_init(void *nvenc_data, void *encoder_config)
{
	...
	*/
	...
	//initializeParams.encodeConfig->encodeCodecConfig.h264Config.outputPictureTimingSEI = 1;
	// 有averageBitRate需要并用作速率控制算法的目标输出比特率。
	// 客户端可以使用NV_ENC_RC_PARAMS::lowDelayKeyFrameScale来控制 I 帧与 P 帧的比率，如果 I 帧最终生成大量比特，这对于避免信道拥塞很有用。设置NV_ENC_CONFIG_H264/ NV_ENC_CONFIG_HEVC::enableFillerDataInsertion = 1以防需要严格遵守比特率
...
#define DEFAULT_BITRATE (1000000u)
	...
	//RateControlParams.vbvBufferSize = g_cfg.get_uint32(ECI_RtcAvgRate) * 1000;
